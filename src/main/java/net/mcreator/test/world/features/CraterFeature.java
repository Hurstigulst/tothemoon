...
public class CraterFeature extends Feature<NoneFeatureConfiguration> {
	public static CraterFeature FEATURE = null;
	public static Holder<ConfiguredFeature<NoneFeatureConfiguration, ?>> CONFIGURED_FEATURE = null;
	public static Holder<PlacedFeature> PLACED_FEATURE = null;

	public static Feature<?> feature() {
		FEATURE = new CraterFeature();
		CONFIGURED_FEATURE = FeatureUtils.register("test:crater", FEATURE, FeatureConfiguration.NONE);
		PLACED_FEATURE = PlacementUtils.register("test:crater", CONFIGURED_FEATURE, List.of());
		return FEATURE;
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("test", "crater"));
		if (template == null)
			return false;
